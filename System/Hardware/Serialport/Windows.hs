...
instance RawIO SerialPort where
  read (SerialPort h _) ptr n =
    return . fromIntegral =<< win32_ReadFile h ptr (fromIntegral n) Nothing
  readNonBlocking _ _ _ = error "readNonBlocking not implemented"
  write (SerialPort h _) ptr n =
    void (win32_WriteFile h ptr (fromIntegral n) Nothing)
...
openSerial ::
     String -- ^ Serial port, such as @COM5@ or @CNCA0@
  -> SerialPortSettings
  -> IO SerialPort
...
-- |Send bytes
send :: SerialPort -> B.ByteString -> IO Int -- ^ Number of bytes actually sent
...
-- |Flush buffers
flush :: SerialPort -> IO ()
flush s@(SerialPort h _) = flushFileBuffers h >> consumeIncomingChars
  where
...
setSerialSettings (SerialPort h _) new_settings = do
  let ct =
        ...
        , Comm.readTotalTimeoutConstant =
            fromIntegral (timeout new_settings) * 100
